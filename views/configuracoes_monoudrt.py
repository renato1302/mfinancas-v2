...
def render_configuracoes():
    ...
    with tab_contas:
        ...
        with st.form("form_conta", clear_on_submit=True):
            ...
            tipo_c = c2.selectbox("Tipo", ["Conta Corrente", "Cartão", "Dinheiro", "Investimento (Liquidez)", "Patrimônio (Imóvel)"])
    ...
    with tab_cats:
        ...
        with st.form("form_hierarquia", clear_on_submit=True):
            ...
            split = st.checkbox("Este Subgrupo permite desmembrar valores? (Ex: Mercado)")
            ...
            with st.expander("✏️ Editar ou 🗑️ Excluir Categoria"):
                ...
                if cat_sel:
                    ...
                    with st.form("form_edit_cat"):
                        ...
                        novo_split = st.checkbox("Permite desmembrar?", value=bool(row_c.get('permite_split', False)))
                        ...
                        if col_btn1.form_submit_button("💾 Salvar Alterações"):
                            if novo_g and novo_sg and novo_sc:
                                executar_query("""
                                                ...
                                                SET grupo=?, subgrupo=?, subcategoria=?, permite_split=?
                                                WHERE grupo=? AND subgrupo=? AND subcategoria=?
                                            """, (novo_g.strip(), novo_sg.strip(), novo_sc.strip(), novo_split,
