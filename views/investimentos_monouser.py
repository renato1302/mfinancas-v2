...
def render_investimentos():
    ...
    with tab_dash:
        ...
        if not df_posicao.empty:
            ...
            c_g1.plotly_chart(px.pie(df_posicao, values='Total Atual (R$)', names='tipo', hole=0.4, title="Por Classe", template="plotly_dark"), use_container_width=True)
            c_g2.plotly_chart(px.pie(df_posicao, values='Total Atual (R$)', names='setor', hole=0.4, title="Por Setor", template="plotly_dark"), use_container_width=True)
            st.write("### 📝 Detalhes da Carteira")
            st.dataframe(df_posicao[['ticker', 'tipo', 'setor', 'qtd_total', 'preco_medio', 'Preço Atual', 'Total Atual (R$)']], use_container_width=True)
        else:
